...
public class AuthenticListeners<Plugin extends AuthenticLibreLogin<P, S>, P, S> {
    ...
    protected void onPostLogin(P player, User user) {
        ...
        if (user == null) {
            // User still not found - might be a race condition. Try by name.
            user = plugin.getDatabaseProvider().getByName(platformHandle.getUsernameForPlayer(player));
        }

        if (user == null) {
            // Cannot proceed without a user record
            plugin.getLogger().error("Could not find user record for " + platformHandle.getUsernameForPlayer(player) + " during post-login!");
            return;
        }
    ...
    protected BiHolder<Boolean, S> chooseServer(
        ...
        // If user is still null and not a Floodgate player, send to limbo for login
